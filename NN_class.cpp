...
class NeuralNetwork{
    ...
    NeuralNetwork(){
        std::cout<<"A new object from NN class was constructed"<<std::endl;
    }
    NeuralNetwork(std::vector<int> v){
        //Eigen::Matrix Temp;
        for(int i=1; i<v.size();i++){
            Eigen::MatrixXf Temp = Eigen::MatrixXf::Random(v[i-1], v[i]);
            weights.push_back(Temp);
        }
        std::cout<<"The weights were randomly initialized: "<<v.size()<<std::endl;
    }
    void print_weights(void){
        for( auto& matrix :weights)
            std::cout<<matrix<<std::endl;
    }
    void print_weights_back(void){
        std::cout<<"New function was called"<<std::endl;
        for(int i=1; i<=weights.size(); i++){
            std::cout<<weights[weights.size()-i]<<std::endl;
        }
    }
...
void save(NeuralNetwork& obj){
    // Object to write in file
    std::ofstream outfile("Input.txt");

    if(outfile){
        outfile<<obj.weights<<std::endl;
        outfile.close();
    }
...
int main()
{
    ...
    int epoch=1;//epoch number for the trainning
